...
MyManager::MyManager(sf::Scalar rate, std::string scenarioPath) : ROSSimulationManager(rate, scenarioPath)
{
    ...
    manuel = new sf::ManualTrajectory();

    timer = nh.createTimer(ros::Duration(1), &MyManager::timerCb, this);
}
...
{
    std::cout << "manuel time" << "\n";
    sf::Vector3 point(inc++, 0, 0);
    sf::Quaternion ori(0, 0, 0, 1);
    manuel->setTransform(sf::Transform(ori, point));
}
...
void MyManager::MakeFishes()
{
    ...
    anim = new sf::AnimatedEntity("LeaderFrame", manuel);
    AddAnimatedEntity(anim);

    // mantraj.setTransform();
